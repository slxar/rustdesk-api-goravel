...
func (as *AuditService) CreateAuditAlarm(alarm *model.AuditAlarm) error {
	return DB.Create(alarm).Error
}

func (as *AuditService) AuditAlarmList(page, pageSize uint, where func(tx *gorm.DB)) (res *model.AuditAlarmList) {
	res = &model.AuditAlarmList{}
	res.Page = int64(page)
	res.PageSize = int64(pageSize)
	tx := DB.Model(&model.AuditAlarm{})
	if where != nil {
		where(tx)
	}
	tx.Count(&res.Total)
	tx.Scopes(Paginate(page, pageSize)).Find(&res.AuditAlarms)
	return
}
