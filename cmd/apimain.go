...
import (
	...
	"github.com/slxar/rustdesk-api-goravel/v3/bootstrap"
	"github.com/slxar/rustdesk-api-goravel/v3/config"
	"github.com/slxar/rustdesk-api-goravel/v3/global"
	"github.com/slxar/rustdesk-api-goravel/v3/lib/cache"
	"github.com/slxar/rustdesk-api-goravel/v3/lib/jwt"
	"github.com/slxar/rustdesk-api-goravel/v3/lib/lock"
	"github.com/slxar/rustdesk-api-goravel/v3/lib/logger"
	"github.com/slxar/rustdesk-api-goravel/v3/lib/orm"
	"github.com/slxar/rustdesk-api-goravel/v3/lib/upload"
...
var rootCmd = &cobra.Command{
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		if cmd.Name() == "backup" {
			...
			config.Init(&configured, global.ConfigPath)
			if configured.Gorm.Type != config.TypeSqlite {
				return fmt.Errorf("backup only supports sqlite; configured database is %s", configured.Gorm.Type)
			}
			if _, err := os.Stat(orm.DefaultSQLitePath); err != nil {
				return fmt.Errorf("backup source %s: %w", orm.DefaultSQLitePath, err)
			}
		}
		InitGlobal()
		return nil
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		bootstrap.Boot(global.Config.Gin.ApiAddr).Start()
	},
}
...
var backupCmd = &cobra.Command{
	...
	Example: "backup ./backups/rustdeskapi-$(date +%F).db",
	Short:   "Create an atomic SQLite database backup",
	Args:    cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		if global.Config.Gorm.Type != config.TypeSqlite {
			return fmt.Errorf("backup only supports sqlite; configured database is %s", global.Config.Gorm.Type)
		}
		return orm.BackupSQLite(global.DB, args[0])
	},
}

func init() {
	rootCmd.PersistentFlags().StringVarP(&global.ConfigPath, "config", "c", "./conf/config.yaml", "choose config file")
	rootCmd.AddCommand(resetPwdCmd, resetUserPwdCmd, backupCmd)
}
func main() {
