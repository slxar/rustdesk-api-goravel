...
// UnmarshalJSON accepts providers that incorrectly encode email_verified as a string.
func (ou *OidcUser) UnmarshalJSON(data []byte) error {
	...
	ou.Name, ou.Email, ou.Sub = payload.Name, payload.Email, payload.Sub
	ou.PreferredUsername, ou.Picture = payload.PreferredUsername, payload.Picture
	ou.VerifiedEmail = false
	raw := bytes.TrimSpace(payload.VerifiedEmail)
	if len(raw) == 0 || bytes.Equal(raw, []byte("null")) {
		return nil
	}
	if err := json.Unmarshal(raw, &ou.VerifiedEmail); err == nil {
		return nil
	}
	var value string
	if err := json.Unmarshal(raw, &value); err != nil {
		return fmt.Errorf("email_verified must be a boolean or string: %w", err)
	}
	parsed, err := strconv.ParseBool(strings.TrimSpace(value))
	if err != nil {
		return fmt.Errorf("email_verified must be a boolean or string: %w", err)
	}
	ou.VerifiedEmail = parsed
	return nil
}
